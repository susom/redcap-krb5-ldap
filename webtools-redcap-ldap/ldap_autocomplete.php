...
if ($exact == "true") {
	$filter="uid=$userid";
} elseif ($exact == "autocomplete") {
	$userid_stripped = preg_replace('/([^@]*)@(.*stanford.edu)/i','${1}',$userid);
	$filter="(|(uid~=$userid_stripped)(susunetid~=$userid_stripped)(mail~=$userid_stripped@stanford.edu)(mail~=$userid_stripped)(sudisplaynamelast~=$userid_stripped)(sudisplaynamefirst~=$userid_stripped))";	//search by sunet or mail

	$filter="(|(uid=*$userid_stripped*)(susunetid=*$userid_stripped*)(sudisplaynamelast=$userid_stripped*)(sudisplaynamefirst=*$userid_stripped*))";	//search by sunet or mail

	$filter="(|(uid=*$userid_stripped*)(susunetid=*$userid_stripped*))";	//search by sunet or mail
